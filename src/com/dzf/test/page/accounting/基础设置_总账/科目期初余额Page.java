...
//			Reporter.log(project + ",输入金额" + num);
			...
			input(getWebElement(By.xpath("//tr[contains(.,'" + subject + "')]/td[@field='" + field
					+ "']/div/table/tbody/tr/td/span/input[1]")), num);
...
//			Reporter.log("点击保存按钮");
			...
			// Reporter.log("获取已输入的金额");
			String flag = getText(
					getWebElement(By.xpath(".//tr[contains(.,'" + subject + "')]/td[@field='" + field + "']")));

//			Reporter.log("判断保存的金额是否等于输入的金额");
			if (Double.parseDouble(flag.replace(",", "")) == Double.parseDouble(num)) {
//				Reporter.log("保存的金额等于修改的金额。修改成功！");
...
//				Reporter.log("点击损益");
			...
			List<WebElement> webElementList = getWebElement(table)
					.findElements(By.xpath(".//tr[contains(.,'" + subject + "')]"));
...
//			Reporter.log("点击固定资产同步按钮");
			click("固定资产期初同步按钮");

//			Reporter.log("点击询问面板的确定按钮");
			click("询问面板-确定按钮");

//			if (getText("提示信息").equals("固定资产同步成功！")) {
//				result = true;
//			}
...
//			Reporter.log("获取提示并判断是否是‘刷新成功’");
			if (getText("提示信息").equals("刷新成功！")) {
//				Reporter.log("提示刷新成功！");
